...
- **پشتیبانی از IPv4 و IPv6** — هر دو نوع رنج در فایل `ip_ranges.txt` قابل استفاده‌اند
- **مصرف RAM بهینه‌شده:** هر IP تنها ۱۶ بایت فضا می‌گیرد. برای ۳۰ میلیون IP تنها ~۴۸۰ مگابایت RAM نیاز است (در نسخه‌های قبل ۱.۵ گیگابایت)
- **تعداد worker قابل تنظیم در حالت Xray:** عددی بین ۱ تا ۱۶ را بر اساس قدرت دستگاه خود انتخاب کنید
- **ذخیره و ادامه‌ی اسکن:** پیشرفت اسکن به‌صورت خودکار ذخیره می‌شود (در حالت Normal هر ۲۰۰۰ IP، در حالت Xray هر ۵۰۰ IP). اگر اسکن به هر دلیلی یا با Ctrl+C توسط خود کاربر قطع شود، دفعه‌ی بعد می‌توانید از همان‌جا ادامه دهید
- **پشتیبانی از لیست‌های بسیار بزرگ:** امکان اسکن چند ده میلیون IP در چند نوبت — کاربر می‌تواند هر بار مقداری اسکن کند و هربار از همان‌جا که متوقف شده ادامه دهد
...
هر خط می‌تواند شامل یک IP منفرد یا یک رنج CIDR باشد. هر دو IPv4 و IPv6 پشتیبانی می‌شوند:
...
بله. می‌توانید رنج‌های IPv6 را در فایل `ip_ranges.txt` قرار دهید و ابزار آن‌ها را همراه با IPv4 اسکن می‌کند.
...
در حالت Normal، پیشرفت اسکن هر ۲۰۰۰ IP یک‌بار و در حالت Xray، هر ۵۰۰ IP یک‌بار در فایل `scan_checkpoint.json` ذخیره می‌شود. دفعه‌ی بعد که ابزار را اجرا کنید، اسکن ناتمام را تشخیص داده و می‌پرسد آیا می‌خواهید ادامه دهید یا از نو شروع کنید.

**آیا می‌توان یک لیست بسیار بزرگ را در چند روز اسکن کرد؟**

بله، کاملاً ممکن است. مثلاً با یک لیست ۳۰ میلیونی می‌توانید هر روز چند صد هزار IP اسکن کنید و هربار از همان‌جایی که متوقف شده‌اید ادامه دهید تا پس از چند روز کل لیست اسکن شود. ابزار از یک سیستم seed تصادفی استفاده می‌کند که تضمین می‌کند در هر ادامه، دقیقاً از همان ایندکسی که قطع شده شروع می‌شود و هیچ IP قبلاً اسکن‌شده‌ای تکرار نمی‌شود.
...
- برای لیست‌های بسیار بزرگ (چند ده میلیون IP) نگران crash یا هنگ نباشید — ابزار بهینه‌سازی شده تا با هر اندازه‌ای از لیست بدون مشکل کار کند
