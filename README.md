...
ابزار پیدا کردن IP‌های تمیز Cloudflare برای Termux (اندروید ARM64)

---

## ✨ ویژگی‌ها

- اسکن هزاران IP از رنج‌های رسمی Cloudflare
- **دو روش اسکن پیشرفته:**
...
```bash
pkg update && pkg upgrade -y && pkg install -y wget unzip && wget https://github.com/4n0nymou3/CF-Clean-IP-Scanner/releases/latest/download/cf-scanner-arm64.zip && unzip cf-scanner-arm64.zip && chmod +x cf-scanner
```
...
**محل نصب و فایل‌ها:**
```
~/CF-Clean-IP-Scanner/          ← پوشه‌ی اصلی برنامه
~/CF-Clean-IP-Scanner/config/   ← پوشه‌ی فایل‌های کانفیگ
~/CF-Clean-IP-Scanner/xray/     ← هسته‌ی Xray
```
...
- اسکن با ۲ کارگر (worker) همزمان انجام می‌شود
...
**روش اول (دانلود مستقیم):**
```bash
# اگر در پوشه‌ی home هستید:
ls
# باید پوشه‌ی config را ببینید. وارد آن شوید:
cd config
ls
# فایل xray_config.txt را باید ببینید
```

**روش دوم (ساخت از سورس):**
```bash
cd ~/CF-Clean-IP-Scanner/config
ls
```
...
# روش دوم (ساخت از سورس):
nano ~/CF-Clean-IP-Scanner/config/xray_config.json
```
...
# روش دوم (ساخت از سورس):
nano ~/CF-Clean-IP-Scanner/config/ip_ranges.txt
```
...
rm -f /data/data/com.termux/files/usr/bin/cf-scanner
```
...
- **حالت Normal** (برای ۶۰۰۰ رنج IP):
  - مرحله‌ی تأخیر: ۲ تا ۳ دقیقه
  - مرحله‌ی سرعت: ۱ تا ۲ دقیقه
- **حالت Xray** (با ۲ worker همزمان):
  - هر IP حدود ۱۵ تا ۲۰ ثانیه طول می‌کشد
