...
- **حفاظت در برابر رنج‌های خیلی بزرگ:** اگر یک رنج CIDR بیش از ۵ میلیون آدرس داشته باشد، به جای crash، پیام خطای واضح نمایش داده می‌شود
...
- **پشتیبانی از لیست‌های بسیار بزرگ:** امکان اسکن چند ده میلیون IP در چند نوبت
...
در حالت Normal، پیشرفت اسکن هر ۲۰۰۰ IP یک‌بار و در حالت Xray، هر ۵۰۰ IP یک‌بار در فایل `scan_checkpoint.json` ذخیره می‌شود. اگر اسکن به هر دلیلی قطع شود (Ctrl+C، خاموش شدن دستگاه، بسته شدن Termux و...)، دفعه‌ی بعد که ابزار را اجرا کنید می‌توانید از همان‌جا ادامه دهید.
...
> **نکته‌ی بسیار مهم برای حالت Xray:** اگر IP‌های یک CDN خاص را در `ip_ranges.txt` قرار دادید، کانفیگ Xray شما **حتماً باید روی همان CDN** تنظیم شده باشد.
...
هر خط می‌تواند شامل یک IP منفرد یا یک رنج CIDR باشد. هم IPv4 و هم IPv6 پشتیبانی می‌شوند:
...
رنج‌های IPv6 با پیشوندهای بزرگ مانند `/32` یا `/48` میلیاردها آدرس دارند و برای اسکن یک‌به‌یک غیرعملی هستند. اگر چنین رنجی وارد کنید، ابزار آن را رد کرده و پیام راهنما نمایش می‌دهد — **دیگر crash نمی‌کند.**

برای IPv6 از subnet‌های کوچک استفاده کنید:
...
> **هم‌خوانی کانفیگ با CDN انتخابی:** کانفیگ Xray شما باید با CDN‌ای که IP‌های آن را در `ip_ranges.txt` قرار داده‌اید **هم‌خوانی داشته باشد**.
...
> **نکته:** به‌روزرسانی فایل‌های `config/` را تغییر نمی‌دهد.
...
خیر. با تغییر فایل `config/ip_ranges.txt` می‌توانید IP‌های هر CDN دیگری مانند Akamai، Fastly، Amazon CloudFront و غیره را اسکن کنید.

**آیا IPv6 هم پشتیبانی می‌شود؟**

بله، اما با محدودیت. رنج‌های IPv6 با پیشوند بزرگ (مانند `/32`) میلیاردها آدرس دارند و قابل اسکن یک‌به‌یک نیستند. از subnet‌های کوچک مثل `/120` یا `/112` استفاده کنید. اگر رنجی خیلی بزرگ باشد، ابزار آن را رد کرده و پیام راهنما نمایش می‌دهد.
...
پیشرفت ذخیره می‌ماند. دفعه‌ی بعد ابزار می‌پرسد آیا ادامه می‌دهید یا از نو شروع می‌کنید.

**آیا می‌توان یک لیست بسیار بزرگ را در چند روز اسکن کرد؟**

بله. سیستم seed تضمین می‌کند در هر ادامه دقیقاً از همان ایندکسی که قطع شده شروع می‌شود و هیچ IP قبلاً اسکن‌شده‌ای تکرار نمی‌شود.

**چرا IP پیدا نمی‌کند؟**

- در ساعات کم‌ترافیک دوباره امتحان کنید
- مطمئن شوید هیچ VPN فعالی ندارید
- در حالت Xray، مطمئن شوید کانفیگ شما با CDN مورد نظر هم‌خوانی دارد
...
یکی از این دو فایل را ویرایش کنید: `config/xray_config.txt` یا `config/xray_config.json`

**پیام: `Skipping 2606:4700::/32 (IPv6 /32): contains ... addresses`**

رنج IPv6 شما خیلی بزرگ است. از subnet کوچک‌تر مثل `/120` استفاده کنید.

**خطا: `No IPs to scan`**

تمام رنج‌های فایل `ip_ranges.txt` رد شده‌اند (احتمالاً همه IPv6 بزرگ هستند). رنج‌های IPv4 اضافه کنید یا subnet IPv6 را کوچک‌تر کنید.
